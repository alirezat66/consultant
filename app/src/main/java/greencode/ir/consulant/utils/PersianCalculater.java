...
public class PersianCalculater {
    ...
    public static String getDayNameDayNumberMonthNameAndHours(long milis){
        PersianDate adviceTime = new PersianDate(milis);
        String timeStr  = adviceTime.dayName()+" "+ adviceTime.getShDay()+" "+ adviceTime.monthName()+" ماه، ساعت "+ PersianCalculater.getHourseAndMin(adviceTime.getTime());
        return timeStr;
    }
    ...
    public static String getDayNameDayAndMonthName(long time){
        PersianDate date = new PersianDate(time);
        return (date.dayName()+" "+(date.getShDay()>=10?date.getShDay()+"":"0"+date.getShDay())+" "+ (date.monthName()));
